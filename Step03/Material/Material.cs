...
public class Metal : Material
{
    ...
    public override bool Scatter(Ray rIn, HitRecord rec, out Vector3D attenuation, out Ray scattered, out float pdf)
    {
        ...
        scattered = new Ray(rec.p, reflected + Fuzz * Mathf.RandomInUnitShpere(), rIn.Time);     //模糊镜面反射 = 镜面反射 + 模糊系数 * 单位球随机点漫反射
...
public class DiffuseLight : Material
{
    ...
    {
        if (rec.normal * rIn.Direction < 0)         //上一章没看到作者写了要翻转法向量，我真是对不起作者orz
